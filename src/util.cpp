...
const char * const MAGPIECOIN_CONF_FILENAME = "magpiecoin.conf";
const char * const MAGPIECOIN_PID_FILENAME = "magpiecoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "magpiecoind -foo=bar
...
#else
    const char* pszModule = "magpiecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MagpieCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MagpieCoin
    // Mac: ~/Library/Application Support/MagpieCoin
    // Unix: ~/.magpiecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MagpieCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MagpieCoin";
#else
    // Unix
    return pathRet / ".magpiecoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MAGPIECOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", MAGPIECOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure MagpieCoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("MagpieCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The MagpieCoin Core developers";
    }
