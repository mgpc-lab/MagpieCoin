...
{
    resize(1100, 550);
    setWindowTitle(tr("MagpieCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a MagpieCoin address"));
...
//	connect(chatAction, SIGNAL(triggered()), this, SLOT(gotoChatPage()));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About MagpieCoin"), this);
    aboutAction->setToolTip(tr("Show information about MagpieCoin"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for MagpieCoin"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("MagpieCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("MagpieCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to MagpieCoin network", "", count));
}
...
{
    QString strTitle = tr("MagpieCoin") + " - ";
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::MGPC, nFeeRequired));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid MagpieCoin address or malformed URI parameters."));
    }
