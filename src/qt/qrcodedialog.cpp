...
{
    QString ret = QString("magpiecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MGPC unit input in lnReqAmount, we generate the URI with MGPC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MGPC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
