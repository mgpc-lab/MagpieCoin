...
const std::string strMessageMagic = "MagpieCoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s MGPC additional fees, %d delta bytes\n",
...
{
    CAmount nSubsidy = 48000 * COIN;    
    int times = nHeight / consensusParams.nSubsidyHalvingInterval;
    if (nHeight <=20)
    {
        nSubsidy = 340874592 * COIN;
    }
    else if (nHeight >20 && nHeight < consensusParams.nSubsidyHalvingInterval)
    {
        nSubsidy = 1 * nSubsidy;
    }
    ...
    else //nHeight>= 2*consensusParams.nSubsidyHalvingInterval
    {
       nSubsidy = 0.5 * nSubsidy;
...
void ThreadScriptCheck() {
    RenameThread("magpiecoin-scriptch");
