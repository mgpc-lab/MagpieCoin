...
#if defined(NDEBUG)
# error "MagpieCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5fb902b0b7831a77d03261848c47ab439132fe91a5f68ed6d7dd78e2f7540285");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 10); // MagpieCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MagpieCoin Signed Message:\n";
...
{
    // MagpieCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 48000 * COIN;
    int times = nHeight / 420000;
    if (nHeight <=20)
    {
        nSubsidy = 340874592 * COIN;
    }
    else if (nHeight >20 && nHeight <420000)
    {
        nSubsidy = 1 * nSubsidy;
    }
        else if( nHeight >=420000 && nHeight < 840000)
        {
                    nSubsidy = 0.5 * nSubsidy;
    }
    else //nHeight>= 840000
    {
    nSubsidy = 0.5 * nSubsidy;
...
static const int64 nTargetTimespan = 4*60*60; // MagpieCoin: 4hrs
static const int64 nTargetSpacing = 2*60; // MagpieCoin: 10 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MagpieCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("magpiecoin-y-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MagpieCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MagpieCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5fb902b0b7831a77d03261848c47ab439132fe91a5f68ed6d7dd78e2f7540285");
    }
...
bool InitBlockIndex() {
   ...
   // fTxIndex = GetBoolArg("-txindex", false);
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].nValue = 48000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049ad6b9626ab1dab2226da2f65cb6e9919545a35a9c5203e26722c9d697f53fc6e6339bef8a8f44951154d0face2b317b22579242a1dd28d5b5b268acb73e4659") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615475500;
        ...
        assert(block.hashMerkleRoot == uint256("0x547c99f02aa1ab3bb9a051393a1f64ccac5a8b9dfcd6ad257b236c784bb925c9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x9c, 0xc7, 0xfe, 0xe7 }; //
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MagpieCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MagpieCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MagpieCoinMiner(CWallet *pwallet)
{
    printf("MagpieCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("magpiecoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running MagpieCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        while (true)
        {
            ...
            while (true)
            {
                yespower_hash(BEGIN(pblock->nVersion), BEGIN(thash));
    ...
    {
        printf("MagpieCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MagpieCoinMiner, pwallet));
}
