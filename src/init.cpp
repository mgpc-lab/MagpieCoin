...
void Shutdown()
{
    ...
    RenameThread("magpiecoin-y-shutoff");
...
// std::cout << "daemon=" << mapArgs["-daemon"] << endl;
        ...
        {
            // First part of help message is specific to magpiecoind / RPC client
            std::string strUsage = _("MagpieCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  magpiecoind [options]                     " + "\n" +
                  "  magpiecoind [options] <command> [params]  " + _("Send command to -server or magpiecoind") + "\n" +
                  "  magpiecoind [options] help                " + _("List commands") + "\n" +
                  "  magpiecoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "magpiecoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect magpiecoind signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: magpiecoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: magpiecoind.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the MagpieCoin Wiki for SSL setup instructions)") + "\n" +
...
{
    RenameThread("magpiecoin-y-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MagpieCoin is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("MagpieCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "MagpieCoin server starting\n");
    ...
    // requested to kill magpiecoin-y-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MagpieCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MagpieCoin to complete") << "\n";
